...
- **智能对话**：自然语言 → Coordinator → A2A 功能 Agent → MCP。
...
首次基线轮询（`notify_on_first_run=false`）只存 BV 号 + URL、不做内容提取；B 站采集用 yt-dlp `extract_flat` 扁平模式，基线数据连 `title` 都只是 BV 号。对已入库但未提取的历史视频，可用**一次性命令**补全（转写/摘要/关键词 + 真实标题/发布时间，不新增任何文件）：
...
3. **首次基线不提取**：`notify_on_first_run=false` 时首轮只建基线不转写；之后轮询只对**新 BV 号**做提取和推送（有真实字幕走 subtitle，无字幕则 ASR）。
