...
> - **A2A**：协调调度 Agent 按意图路由任务；任务完成后反问用户是否生成简报（A2A 交接）
> - **MCP**：Agent 调用资讯源、检索、数据与推送工具
...
| ④ 持续监控某账户 | 注册、定时增量检查、视频转写、入库通知 | Coordinator → AccountMonitorAgent → 监控服务/VideoAgent |
...
flowchart TD
    U[用户查询] --> M{查询模式}
    M -->|在线| C[Coordinator Agent<br/>意图识别 · 参数抽取]
    C -->|A2A HTTP| CA[Collector Agent]
    CA --> CM[Collect MCP :8004]
    C -->|热点任务 A2A HTTP| PA[Processor Agent]
...
| 入口 | 调用路径 | 特点 |
| --- | --- | --- |
| 对话入口（LLM 编排） | 用户自然语言 → 协调调度 Agent 意图分析 → 调用流水线 | 灵活，支持任意表述；每次由 LLM 规划 |
| 前端入口（功能直选） | 前端功能按钮（热点/最新/账户）→ 直接调用 `task_pipelines` 对应函数 | 快、确定、不走 LLM 意图识别 |

`task_pipelines` 作为**底层可复用的功能执行单元**，两个入口共用：对话入口由协调器调用，前端入口直接调用。
...
│   ├── coordinator_agent.py      # 协调调度 Agent：意图识别 · 路由 · A2A 简报交接
...
│   ├── video_agent.py            # 视频 Agent：字幕 · ASR · 内容摘要（A2A :8008）
...
├── account_monitor/                # 监控状态、发布去重、视频处理与通知
├── output/                       # 简报与展示产物
...
| 层 | 说明 | 目录 |
| --- | --- | --- |
| 任务路由 | 协调调度 Agent 经 `intent_agent()` 识别后，通过真实 HTTP A2A 委派 Collector / Processor / Publisher / AccountMonitor；持续监控业务由下游 AccountMonitorAgent 完成 | `agents/coordinator_agent.py` + `agents/intent_agent.py` + `a2a/protocol.py` |
| 前端直连 | 前端功能选择 → 直接调用 `task_pipelines` 对应函数 | `task_pipelines/` |
...
| Agent → Tool | Agent 方法 → `mcp_servers/mcp_access.py` 网关（`sync_call` 桥）→ 官方 `mcp.client.streamable_http.streamablehttp_client` 连远端 MCP 服务器 → `ClientSession.initialize()` → `call_tool` 调注册工具；远端不可达时自动降级进程内直调 `tools.*`。另有 `mcp_agent_call()` 用 `load_mcp_tools` + LangChain 工具调用 Agent 由 LLM 驱动 MCP 工具 | `agents/` → `mcp_servers/mcp_access.py` → `mcp_servers/mcp_*.py` → `tools/registry.py` → `tools/` |
...
- Agent 服务可直接跑：`python -m agents.coordinator_agent` / `collector_agent` / `processor_agent` / `publisher_agent` / `video_agent` / `account_monitor_agent`。VideoAgent 监听 `127.0.0.1:8008`，AccountMonitorAgent 监听 `127.0.0.1:8009`。
...
- 对话路径：自然语言 → 协调调度 Agent（LLM 意图识别）→ A2A 功能 Agent → MCP 网关（不可达
  自动降级进程内直调 `tools.*`）→ 结果展示 → A2A 反问是否生成简报并推送。
...
已配置示例空间 `https://space.bilibili.com/312249633/video`。链路：

```text
定时轮询 → Collect MCP 读取最近视频 → MySQL BV 号去重
         → 发现新视频 → A2A VideoAgent(:8008) → Video MCP(:8007)
         → 字幕优先 / 无字幕则音频 ASR
         → LLM 摘要和关键词 → Publish MCP 简报推送
```

MySQL 表：`monitored_accounts` 保存最后检查与错误，`account_posts` 保存发布、转写、摘要和通知状态。首次运行默认只建立基线，之后只处理新 BV 号，不会重复转写或推送。
...
# 视频 A2A Agent（发现新视频后接收处理委派）
"D:\ceshi\Anaconda3\envs\news_feed\python.exe" -m agents.video_agent
```
