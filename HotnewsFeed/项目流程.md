...
`task_pipelines` 由 `operations.py` 提供给确定性按钮/斜杠命令；Coordinator 不再包含任何固定业务流水线。
...
- **A2A 实现**：协议类型（`Message` / `MessageRole` / `AgentCard` / `A2AServer`）来自官方包 **python_a2a**；`a2a/protocol.py` 只封装 `send_task` / `parse_task` / `delegate` / `reply_text`
...
│   └── protocol.py               # send_task / parse_task / delegate / reply_text
│
├── agents/                       # 多智能体层（均直接继承 A2AServer + 标准 AgentCard）
│   ├── coordinator_agent.py      # 唯一协调入口：自主 Agent Loop + A2A dispatch
...
├── operations.py                 # 明确按钮/斜杠命令的确定性操作层（不进入 Agent 推理）
...
| 层 | 说明 | 目录 |
| --- | --- | --- |
| 任务路由 | Coordinator 从 AgentCard 构建能力目录；LLM 每轮只产生一个 delegate/ask_user/finish 动作，执行结果写入 Observation 后再规划。不存在旧意图分类或固定链路回退 | `agents/runtime/` + `agents/coordinator_agent.py` + `a2a/protocol.py` |
...
| Agent → Agent | A2A 协议类型来自官方包 **python_a2a**（`Message` / `AgentCard` / `A2AServer`）；统一使用 `a2a.delegate()` 委派 | `a2a/` + `agents/` |
...
- **自主运行时**：`[agent_runtime]`，最大迭代数、Agent 调用数、总时限和 Trace 展示
